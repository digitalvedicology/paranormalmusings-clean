 ...
 *
 * This frontend-1 is a testing instance that ONLY fetches from admin with no fallbacks.
 * If the admin is unreachable, pages will error. Use this to verify real-time sync.
 */
...
async function fetchDoc(): Promise<ContentDoc> {
  ...
  if (!response.ok) throw new Error(`[content] Admin API returned ${response.status}`)

  const doc = await response.json()
  if (!Array.isArray(doc.categories) || !Array.isArray(doc.posts)) {
    throw new Error('[content] Admin returned unexpected payload')
  }

  console.log('[content] Successfully fetched from admin')
  return doc
}
