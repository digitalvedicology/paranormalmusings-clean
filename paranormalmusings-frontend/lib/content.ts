...
async function fetchDoc(): Promise<ContentDoc> {
  try {
    ...
    console.warn(`[content] admin fetch failed, falling back to static: ${(error as Error).message}`)
    try {
      ...
      console.warn(`[content] static content also failed, using seed`)
