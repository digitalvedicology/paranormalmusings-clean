 ...
 * Wire `onSubmit` to a real list provider later; for now it just acknowledges
  ...
  if (variant === 'sidebar') {
    return (
      <form className="mt-4 grid gap-2.5" onSubmit={submit}>
        ...
        <button
          type="submit"
          className="h-11 rounded-xl bg-gold-500 text-white text-[14px] font-semibold hover:bg-gold-600 transition"
        >
          {done ? 'You are on the list' : 'Subscribe'}
        </button>
      ...
      <form className="mt-6" onSubmit={submit}>
        ...
        <div className="mt-3 flex items-center gap-2 rounded-full bg-white/[0.06] border border-white/10 pl-4 pr-1.5 py-1.5">
          <input
            ...
            className="flex-1 h-9 bg-transparent text-[14px] text-white placeholder:text-white/35 outline-none"
          />
          <button
            type="submit"
            aria-label="Subscribe"
            className="grid place-items-center w-9 h-9 rounded-full bg-gold-500 hover:bg-gold-600 transition"
          >
            <ArrowRight />
          </button>
        </div>
    ...
    <>
      <form className="mt-4 flex flex-col sm:flex-row gap-2.5 max-w-lg" onSubmit={submit}>
        <input
          ...
          placeholder="Enter your email"
          aria-label="Email address"
          className="flex-1 h-12 px-4 rounded-full bg-paper border border-white/70 text-[14.5px] text-ink placeholder:text-muted outline-none focus:ring-2 focus:ring-gold-300"
        />
        <button
          type="submit"
          className="link-arrow inline-flex items-center justify-center gap-2 h-12 px-6 rounded-full bg-ink text-white text-[14px] font-semibold hover:bg-night-700 transition"
        >
          Join the list
          <ArrowRight />
        </button>
